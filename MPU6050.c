...
{
	HAL_I2C_Mem_Write(&hi2c1, MPU6050_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, &val, 1, 100);
}

void MPU6050_Writebytes(uint8_t reg_addr, uint8_t len, uint8_t* data)
{
	HAL_I2C_Mem_Write(&hi2c1, MPU6050_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, 100);
}

void MPU6050_Readbyte(uint8_t reg_addr, uint8_t* data)
{
	HAL_I2C_Mem_Read(&hi2c1, MPU6050_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, 1, 100);
}

void MPU6050_Readbytes(uint8_t reg_addr, uint8_t len, uint8_t* data)
{
	HAL_I2C_Mem_Read(&hi2c1, MPU6050_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, 100);
}
...
void MPU6050_Initialization(void)
{
	...
	MPU6050_Writebyte(MPU6050_SMPRT_DIV, 0x09); // ACC output rate is 1kHz, GYRO output rate is 8kHz
	//MPU6050_Writebyte(MPU6050_SMPRT_DIV, 0x07); //set data rate to 1kHz
	//MPU6050_Writebyte(MPU6050_SMPRT_DIV, 0x27); // Sample Rate = 200Hz
	//MPU6050_Writebyte(MPU6050_SMPRT_DIV, 0x00); // Sample Rate = 200Hz
	//MPU6050_Writebyte(MPU6050_SMPRT_DIV, 0x4F);
	HAL_Delay(50);

	//FSYNC and DLPF setting
	/*DLPF is set to 0*/ //0x01 -> 184 Hz, MPU6050_SMPRT_DIV -> 0x09
    MPU6050_Writebyte(MPU6050_CONFIG, 0x01);
	HAL_Delay(50);

	//Enable X and Y, disable Z and gyro
	MPU6050_Writebyte(MPU6050_PWR_MGMT_2, 0x08);
	HAL_Delay(50);
	...
	MPU6050_Writebyte(MPU6050_ACCEL_CONFIG, FS_SCALE_ACC<<0);
	HAL_Delay(50);
...
{
	uint8_t data[14];
	MPU6050_Readbytes(MPU6050_ACCEL_XOUT_H, 4, data); // set for 14 when using all unit
...
{
	//printf("LSB_Sensitivity_GYRO: %f, LSB_Sensitivity_ACC: %f\n",LSB_Sensitivity_GYRO,LSB_Sensitivity_ACC);
	mpu6050->acc_x = (mpu6050->acc_x_raw / LSB_Sensitivity_ACC) - 0.0325;
	mpu6050->acc_y = (mpu6050->acc_y_raw / LSB_Sensitivity_ACC) + 0.0105;
