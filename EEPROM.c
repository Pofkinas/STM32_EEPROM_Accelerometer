...
#define EEPROM_ADDR (0x50 << 1) // Primary address in 8-bit format
...
void EEPROM_Write_NUM (uint16_t page, uint16_t offset, float fdata)
{
    ...
    memcpy(buffer, &fdata, sizeof(fdata));
    EEPROM_Write(page, offset, buffer, sizeof(buffer));
}
...
float EEPROM_Read_NUM (uint16_t page, uint16_t offset)
{
    ...
    EEPROM_Read(page, offset, buffer, sizeof(buffer));
 ...
 * @page is the number of page to check
 */
bool EEPROM_PageHasData(uint16_t page) {
    uint8_t buffer[PAGE_SIZE];
    EEPROM_Read(page, 0, buffer, PAGE_SIZE);
    for (int i = 0; i < PAGE_SIZE; i++) {
        if (buffer[i] != 0xFF) {  // Assuming 0xFF is the default erased state
            return true;
        }
    }
    return false;
...
void EEPROM_PageErase (uint16_t page)
{
	...
	/*if (EEPROM_PageHasData(page)) {
        uint8_t blankPage[PAGE_SIZE];
        memset(blankPage, 0xFF, PAGE_SIZE);
        EEPROM_Write(page, 0, blankPage, PAGE_SIZE);
    }*/
